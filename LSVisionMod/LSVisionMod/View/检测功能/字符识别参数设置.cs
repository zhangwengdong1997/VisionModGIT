...
namespace LSVisionMod.View.检测功能
{
    public partial class 字符识别参数设置 : UserControl, I检测参数设置
    {
        ...
        public void Initial(List<Parameter> inParameters)
        {
            string ocrClassifier = inParameters[0].value as string;
            ...
            if(lists is null)
            {
                JArray jlists = inParameters[4].value as JArray;
                row1 = (float)jlists[0];
                column1 = (float)jlists[1];
                row2 = (float)jlists[2];
                column2 = (float)jlists[3];
            }
            else
            {
                row1 = lists[0];
        ...
        public void SetHalconImage(HObject ho_image)
        {
            ...
            HOperatorSet.CopyImage(ho_image, out hImage);
        }
        ...
        private void btn选择区域_Click(object sender, EventArgs e)
        {
            ...
            HOperatorSet.DrawRectangle1(hWindow, out HTuple hv_row1, out HTuple hv_column1, out HTuple hv_row2, out HTuple hv_column2);
            HOperatorSet.GenRectangle1(out hROI, hv_row1, hv_column1, hv_row2, hv_column2);

            if(hImage != null)
            {
                HOperatorSet.DispObj(hImage, hWindow);
            }
            HOperatorSet.DispObj(hROI, hWindow);
        ...
        public void WriteSample(string path, string name)
        {
            HObject regionDilation;
            HObject imageReduced;
            HObject imagePart;
            HOperatorSet.DilationCircle(hROI, out regionDilation, 3);
            HOperatorSet.ReduceDomain(hImage, regionDilation, out imageReduced);
            HOperatorSet.CropDomain(imageReduced, out imagePart);
            HOperatorSet.WriteImage(imagePart, "bmp", 0, path + "\\s_" + name);
            ...
            if (hv_Channels.D == 1)
            {
                HOperatorSet.CopyImage(hImage, out ho_DupImage1);
                HOperatorSet.CopyImage(hImage, out ho_DupImage2);
                HOperatorSet.CopyImage(hImage, out ho_DupImage3);
            }
            else if (hv_Channels.D == 3)
            {
                HOperatorSet.Decompose3(hImage, out ho_DupImage1, out ho_DupImage2, out ho_DupImage3);
            }
            else
            {
            ...
            HOperatorSet.ErosionRectangle1(hROI, out ho_RegionErosion, 5, 5);
            HOperatorSet.Difference(hROI, ho_RegionErosion, out ho_RegionDifference);
            ...
            HOperatorSet.WriteImage(ho_ComposeImage, "bmp", 0, path + "\\b_" + name);
        ...
        {
            if(e.TabPage.Text == "识别区域" && hROI != null)
            {
                HOperatorSet.SetDraw(hWindow, "margin");
                HOperatorSet.SetColor(hWindow, "red");
                HOperatorSet.DispObj(hROI, hWindow);
